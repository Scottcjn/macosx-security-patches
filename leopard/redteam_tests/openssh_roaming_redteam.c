/*
 * CVE-2016-0777 OpenSSH Roaming Red Team Test (Leopard 10.5)
 *
 * Drives the roaming-disable / resume-bounds guard with the server-driven
 * over-read. Applies to a *ported* modern OpenSSH (>=5.4) on Leopard; the
 * 4.x system ssh predates client roaming and is not vulnerable. The guard
 * is included so Leopard builds of current OpenSSH are safe by default.
 */
#include <stdio.h>
#include <stddef.h>
...
int main(void) {
    printf("=== CVE-2016-0777 OpenSSH Roaming Red Team Test (Leopard) ===\n\n");
