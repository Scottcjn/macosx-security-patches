#!/bin/bash
# Build Snow Leopard (10.6) Security Patches Package
# Creates installer for Mac OS X 10.6.8 Snow Leopard (Intel only)
#
...
PACKAGE_NAME="SnowLeopardSecurityPatches"
VERSION="1.0"
IDENTIFIER="com.elya.snowleopard-security-patches"

echo "=== Snow Leopard Security Patches Builder ==="
echo "Building for Mac OS X 10.6.x (Intel: i386 + x86_64)"
echo ""
...
if [ "$ARCH" = "i386" ] || [ "$ARCH" = "x86_64" ]; then
    ...
    echo "Building Intel binary (i386 + x86_64)"
else
    ARCH_FLAGS="-arch i386 -arch x86_64"
    echo "Building Intel binary (i386 + x86_64)"
fi

CFLAGS="-O2 -Wall $ARCH_FLAGS -mmacosx-version-min=10.6"
LDFLAGS="-dynamiclib $ARCH_FLAGS -mmacosx-version-min=10.6"
...
mkdir -p build_root/Library/Security/SnowLeopardPatches
mkdir -p build_root/System/Library/LaunchDaemons
mkdir -p build_root/etc/profile.d

# Copy built dylibs
cp build/patches/*.dylib build_root/Library/Security/SnowLeopardPatches/

# Create combined security dylib loader script
cat > build_root/etc/profile.d/snowleopard_security.sh << 'EOF'
# Snow Leopard Security Patches Loader
# CVE fixes for Mac OS X 10.6.x
#
# Loaded dylibs:
...
SNOWLEOPARD_PATCHES="/Library/Security/SnowLeopardPatches"

if [ -d "$SNOWLEOPARD_PATCHES" ]; then
    export DYLD_INSERT_LIBRARIES="${SNOWLEOPARD_PATCHES}/dns_randomizer.dylib:${SNOWLEOPARD_PATCHES}/tcp_isn_randomizer.dylib:${SNOWLEOPARD_PATCHES}/hfs_overflow_guard.dylib:${SNOWLEOPARD_PATCHES}/font_parsing_guard.dylib:${SNOWLEOPARD_PATCHES}/iokit_bounds_guard.dylib"
fi
EOF

# Create launchd plist for mDNSResponder (DNS randomization)
cat > build_root/System/Library/LaunchDaemons/com.elya.snowleopard-dns-security.plist << 'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.elya.snowleopard-dns-security</string>
    <key>EnvironmentVariables</key>
    <dict>
        <key>DYLD_INSERT_LIBRARIES</key>
        <string>/Library/Security/SnowLeopardPatches/dns_randomizer.dylib</string>
    </dict>
...
#!/bin/bash
# Snow Leopard Security Patches Postinstall
# Sets permissions and loads security modules

echo "Installing Snow Leopard Security Patches..."

# Set permissions
chmod 755 /Library/Security/SnowLeopardPatches/*.dylib
chmod 644 /Library/Security/SnowLeopardPatches/*.dylib
chown -R root:wheel /Library/Security/SnowLeopardPatches
chmod 755 /Library/Security/SnowLeopardPatches

chmod 644 /etc/profile.d/snowleopard_security.sh
chmod 644 /System/Library/LaunchDaemons/com.elya.snowleopard-dns-security.plist

# Create profile.d if needed
mkdir -p /etc/profile.d

# Add to profile
PROFILE_LINE="[ -f /etc/profile.d/snowleopard_security.sh ] && . /etc/profile.d/snowleopard_security.sh"
if ! grep -q "snowleopard_security.sh" /etc/profile 2>/dev/null; then
    echo "" >> /etc/profile
    echo "# Snow Leopard Security Patches" >> /etc/profile
    echo "$PROFILE_LINE" >> /etc/profile
fi

# Load the launch daemon
launchctl load /System/Library/LaunchDaemons/com.elya.snowleopard-dns-security.plist 2>/dev/null || true

echo ""
echo "Snow Leopard Security Patches installed successfully!"
echo ""
echo "CVEs Patched:"
...
if [ -d /Library/Security/SnowLeopardPatches ]; then
    echo "Backing up existing patches..."
    cp -r /Library/Security/SnowLeopardPatches /Library/Security/SnowLeopardPatches.backup.$(date +%Y%m%d)
fi
exit 0
EOF
...
<dict>
    ...
    <string>Snow Leopard Security Patches</string>
...
<dict>
    ...
    <string>Security patches for Mac OS X 10.6.x Leopard
...
if command -v hdiutil &> /dev/null; then
    ...
    hdiutil create -volname "Snow Leopard Security Patches" \
...
echo ""
echo "Uninstallation:"
echo "  sudo rm -rf /Library/Security/SnowLeopardPatches"
echo "  sudo rm /System/Library/LaunchDaemons/com.elya.snowleopard-dns-security.plist"
echo "  sudo rm /etc/profile.d/snowleopard_security.sh"
